...
- **Цена полная:** CAD $190 + $9.50 GST (Alberta 5%) = $199.50
- **Депозит:** CAD $95.00 через Interac e-Transfer
- **Баланс в день съёмки:** CAD $104.50
...
- [x] **Race condition** — исправлено: `BEGIN IMMEDIATE` в `/reserve`
- [x] **Endpoint /expired** — существует, работает
- [x] **Email подтверждение** — работает через Himalaya CLI
- [ ] **Нет Google Calendar** — события не создаются автоматически
- [ ] **Нет SMS-напоминаний** — клиенты могут забыть о съёмке
- [x] **Поддержка нескольких дат** — через `events.yaml`
...
- ❌ **Ценообразование** — $95.00 депозит, $199.50 полная сумма с GST, $104.50 баланс
